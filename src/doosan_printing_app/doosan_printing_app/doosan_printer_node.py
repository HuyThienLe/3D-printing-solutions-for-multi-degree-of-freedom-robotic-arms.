...
class SmartPrinter5AxisPro(Node):
    def __init__(self):
        ...
        self.robot_status = "Waiting for connections..."
        self.selected_file = None
        self.total_points = 0
        self.current_point = 0
        self.print_start_time = None
        
        # KHỞI TẠO GUI ĐẦU IN (Chỉ khởi tạo object, chưa hiển thị)
        ...
        # =========================================================
        # FIX LỖI TREO GUI: Chạy kết nối phần cứng bằng luồng ngầm
        # =========================================================
        threading.Thread(target=self._ket_noi_phan_cung_ngam, daemon=True).start()

        # Update initial robot status (delay to ensure GUI ready)
        def _update_initial_status():
            time.sleep(1.0) # Đợi GUI bật lên hẳn
            try:
                if self.gui_panel and self.gui_panel.root:
                    self.gui_panel.update_robot_status(self.robot_status)
                    self.gui_panel.update_status_display("Đang tìm Robot và Arduino...")
            except:
                pass
        
        threading.Thread(target=_update_initial_status, daemon=True).start()

    # --- HÀM KẾT NỐI CHẠY NGẦM KHÔNG LÀM TREO GIAO DIỆN ---
    def _ket_noi_phan_cung_ngam(self):
        """Hàm này tự động dò Arduino và Robot ở chế độ nền"""
        # 1. Kết nối Arduino
        try:
        ...
        # 2. Kết nối Robot (Hàm này có vòng lặp while liên tục)
        if not self.controller.wait_cho_robot_ket_noi(1.0): 
            ...
            if self.gui_panel and getattr(self.gui_panel, 'root', None):
                self.gui_panel.update_robot_status(self.robot_status)
                self.gui_panel.update_status_display("Robot đã sẵn sàng. Chọn file để in!")
    ...
    def cho_arduino_san_sang(self):
        if self.arduino is None or not self.arduino.is_open:
            self.get_logger().warn("⚠️ Bỏ qua chờ nhiệt độ vì không có kết nối Arduino (Chế độ mô phỏng)")
            return True
    ...
    def run_gcode_file(self, filename: str, start_line: int = 1):
        ...
        for i, pt_base in enumerate(points):
            ...
            # =================================================================
            # TÍNH NĂNG MỚI: BẢO VỆ QUÁ NHIỆT (THERMAL SAFEGUARD)
            # Dừng đồng bộ cả Robot và Đầu đùn nếu nhiệt độ vọt lố quá 5 độ C
            # =================================================================
            while not self.should_stop and rclpy.ok():
                try:
                    curr_t = float(self.current_temp_str)
                except ValueError:
                    curr_t = self.target_temp
                
                # Nếu nhiệt độ thực > Nhiệt độ set + 5 độ C
                if curr_t > (self.target_temp + 5.0):
                    warn_msg = f"⚠️ QUÁ NHIỆT ({curr_t:.1f}°C). Tạm dừng chờ nguội..."
                    self.get_logger().warn(warn_msg)
                    if self.gui_panel and self.gui_panel.root:
                        self.gui_panel.update_status_display(warn_msg)
                    time.sleep(1.0) # Ngủ 1 giây rồi check lại nhiệt độ
                else:
                    break # Nhiệt độ an toàn (< 255 độ), thoát vòng lặp để tiếp tục in
            # =================================================================
            ...
            if hasattr(pt_exec, 'e') and pt_exec.e != 0.0:
                flow_rate = 1.0  # Tăng lượng đùn lên 120%
                e_val = pt_exec.e * flow_rate  
                f_val_mm_min = pt_exec.speed * 60.0 
                arduino_cmd = f"G1 E{e_val:.4f} F{f_val_mm_min:.1f}"
                self.gui_lenh_arduino(arduino_cmd)

            success = self.controller.gui_lenh_move_line(pt_exec, step=i)
            if success is False:
                self.get_logger().error("Lỗi: Quỹ đạo bị từ chối. Dừng in!")
                if self.gui_panel and self.gui_panel.root:
                    self.gui_panel.update_status_display("Lỗi: Quỹ đạo bị từ chối")
                self.should_stop = True
                break
...
def main(args=None):
    ...
    try:
        ...
        if enable_gui:
            # Khởi tạo GUI trực tiếp trong main thread để tránh lỗi Segmentation fault
            node.get_logger().info(">>> Khởi động GUI... <<<")
            try:
                gui_started = True
                node.gui_panel.start_gui()  # Blocking call, sẽ giữ chương trình chạy cho đến khi tắt cửa sổ
            except Exception as e:
        ...
        # Nếu GUI không bật hoặc khởi động lỗi, vào vòng lặp chờ vô tận
        if not enable_gui or not gui_started:
