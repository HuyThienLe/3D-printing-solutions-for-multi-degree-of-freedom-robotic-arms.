...
try:
    ...
    x11.XInitThreads()
    print("[GUI] XInitThreads() OK")
except Exception as e:
    print(f"[GUI] XInitThreads() warning: {e}")
...
class PrintHeadControlPanel:
    ...
    def start_gui(self):
        ...
        try:
            ...
            file_frame = tk.LabelFrame(main_frame, text=" [FILE] Select G-code File ", font=("Arial", 11, "bold"), 
                                       bg="#f0f0f0", fg="#1a1a2e")
            ...
            temp_display = tk.LabelFrame(monitor_frame, text=" CURRENT TEMPERATURE ", font=("Arial", 11, "bold"),
            ...
            # Status Display
            status_display = tk.LabelFrame(monitor_frame, text=" SYSTEM STATUS ", font=("Arial", 11, "bold"),
            ...
            time_display = tk.LabelFrame(monitor_frame, text=" REMAINING TIME ", font=("Arial", 11, "bold"),
            ...
            print("[GUI] Control buttons OK")

            # ===== ROW 5: TEMPERATURE CONTROL =====
            temp_section = tk.LabelFrame(main_frame, text=" HEATER TEMPERATURE (0-300 C) ", 
            ...
            print("[GUI] Temperature section OK")
            ...
            self.is_ready = True
            # Bring window to front sau khi mainloop sẵn sàng (an toàn, không gây crash)
    ...
    def update_temp_display(self, temp_str):
        """Update temperature display (thread-safe)"""
        if self.root and self.lbl_temp and self.is_ready:
            def _update():
                try:
    ...
    def update_status_display(self, status_text):
        ...
        if self.root and self.lbl_status and self.is_ready:
            def _update():
                try:
    ...
    def update_robot_status(self, robot_status):
        """Update robot status display (thread-safe)"""
        if self.root and self.lbl_robot_status and self.is_ready:
            def _update():
                try:
    ...
    def update_progress_display(self, phan_tram):
        """Update progress bar and label with percentage (thread-safe)"""
        if self.root and self.progress_bar and self.is_ready:
            def _update():
                try:
    ...
    def update_estimated_time(self, time_str):
        """Update estimated remaining time (thread-safe)"""
        if self.root and self.lbl_time and self.is_ready:
            def _update():
                try:
    ...
    def update_speed_display(self, speed_text):
        """Update speed display (thread-safe)"""
        if self.root and self.lbl_speed and self.is_ready:
            def _update():
                try:
    ...
    def on_temp_changed(self, value):
        ...
        if not self.is_ready:
            return  # Chặn gọi khi GUI chưa khởi tạo xong
